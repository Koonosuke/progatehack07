...
import SignChatHeader from "@/components/SignChatHeader";
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const idToken = localStorage.getItem(
          Object.keys(localStorage).find((key) => key.endsWith(".idToken")) ||
            ""
        );
      ...
      try {
        ...
        const nameResponse = await fetch(
          `${API_BASE_URL}/getCurrentUserName?sub=${userSub}`
        );
        if (!nameResponse.ok)
          throw new Error("現在のユーザー名の取得に失敗しました");
  ...
  if (isLoading)
    return (
      <div
        ...
        style={{ backgroundColor: "rgba(59, 42, 120, 1)" }}
      >
        <SignChatHeader />
        読み込み中...
      </div>
