...
export default function CallPage() {
  ...
  useEffect(() => {
    ...
    const initializeMediaPipe = async () => {
      ...
      await Promise.all([
        loadScript(
          "https://cdn.jsdelivr.net/npm/@mediapipe/holistic@0.5.1675469242/holistic.min.js"
        ),
        loadScript(
      ...
      const holistic = new Holistic({
        locateFile: (file: string) =>
          `https://cdn.jsdelivr.net/npm/@mediapipe/holistic@0.5.1675469242/${file}`,
      });
      holistic.setOptions({
        modelComplexity: 1,
  ...
  const predictAction = () => {
    ...
    fetch(`${baseURL}/predict`, {
      ...
      .then((data) => {
        alert(
          `予測: ${data.label}(信頼度: ${Math.round(data.confidence * 100)}%)`
        );
      })
    ...
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-start py-8 px-4">
      ...
      <div className="flex flex-col lg:flex-row gap-6 w-full max-w-6xl justify-center">
        ...
        <video
          ...
          autoPlay
          playsInline
          className="w-full lg:w-1/2 bg-black rounded-lg shadow-lg border border-gray-700"
        />
      </div>
